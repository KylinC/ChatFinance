...
PYTHON_PATH=$(jq -r '.base_python' $CONFIG_FILE)
ELASTIC_SEARCH_PATH=$(jq -r '.sever_path.elastic_search' $JSON_FILE)
WEAVIATE_PATH=$(jq -r '.sever_path.weaviate' $JSON_FILE)
LLM_PATH=$(jq -r '.models_path.chatglm2-6b' $JSON_FILE)
TEXT_VEC=$(jq -r '.models_path.text2vec-base-chinese-paraphrase' $JSON_FILE)

# 启动 text2vec model (for WEAVIATE)
cd "$BASE_PATH/$TEXT_VEC"
$PYTHON_PATH jina_server.py
echo "text2vec model start!"

# 启动 elastic search
cd "$BASE_PATH/$ELASTIC_SEARCH_PATH"
docker-compose up -d
echo "elastic DB start!"

# 启动 chatgml-6b
cd "$BASE_PATH/$LLM_PATH"
$PYTHON_PATH jina_server.py
echo "gml-6b model start!"

# 启动 weaviate
cd "$BASE_PATH/$WEAVIATE_PATH"
docker-compose up -d
